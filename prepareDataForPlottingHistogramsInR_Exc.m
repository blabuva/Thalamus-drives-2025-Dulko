function prepareDataForPlottingHistogramsInR_Exc(neatDataWithPExc,strainOfInterest) 
...
%% Remove Dont Analyze 
idxDA = find(strcmp(neatDataWithPExc(:,1), 'DontAnalyze')); % Find the row index
neatDataWithPExc(idxDA, :) = []; % Remove the row

%% Save  S E I Z U R E  data for R 
% Convert cell array to a table
brainStructures = neatDataWithPExc(:,1); % First column (brain structure names)
SWDhistcounts = neatDataWithPExc(:,3); % Second column for seizure data (1x100 doubles)
...
% Save to CSV 
fileName = sprintf('Exc_histogramsSWDs_%s.csv', strainOfInterest); 
...
% save for R 
% Convert cell array to a table
nonSWDshistounts = neatDataWithPExc(:,2); % Second column for non seizures (1x100 doubles)
...
% Save to CSV
fileName = sprintf('Exc_histogramsNONSWDs_%s.csv', strainOfInterest); 
fullFilePath = fullfile(folderPath, fileName); % Combine folder and filename
writetable(dataTable, fullFilePath); % Save the file
