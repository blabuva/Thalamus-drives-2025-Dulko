...
function [AC, OI, swdpeth, trpeth] = ESA_PETHandXCorr_AllChans(mouseID)
...
swdPD = 4; % peri-SWD duration [seconds] (i.e. how long to look around SWD starts and ends)
...
while checkFlag
    ...
    elseif (swdIDX(end,2)) > numel(timevec)
        swdIDX(end,:) = [];
...
for chi = 1:256
    ...
    chanClock = tic;
    fprintf('Channel %d processing...',chi);
    ESAfp = sprintf('%s00%d\\ch%d.bin',locDir,mouseID,chi);
    fid = fopen(ESAfp);
