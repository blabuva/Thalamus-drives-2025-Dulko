%% 
function SPIKYscratch(xdir)
% xdir = '\\172.28.76.244\probeX\intanData\ela\markTemp\0022\analyzedData';
spikeArray = psr_makeSpikeArray(xdir);

%% ---- TRANSFORM CODE BELOW INTO FUNCTION ---- %%
% Restrict spikes to just intra-ictal times
% -- NEED TO RETRIEVE CORRESPONDING LFP -- %
hgt = numel(spikeArray); % height of spike matrix
tmin = 173; % minimum time, seconds (e.g. start of seizure)
tmax = 187; % max time, seconds (e.g. end of seizure))
for ni = 1:hgt
    keepLog{ni} = spikeArray{ni}> tmin & spikeArray{ni} < tmax;
end
...
for ni = 1:hgt
    spikesToKeep = spikeArray{ni}(keepLog{ni});
    spikeSubArray{ni} = spikesToKeep';
    spikesNew(ni,1:numel(spikesToKeep)) = spikesToKeep;
end

keep spikesNew spikeSubArray tmin tmax

%% Load SPIKY workspace data
load('C:\Users\Scott\Documents\SPIKY_30_Apr_2021\SPIKY\SPIKY_ws.mat');
