function   [correlationMatrices, averageMatrix] = CorrCoef_240212(spikesDuringSeizures)
% parent function: MasterCodeSynchrony_241109.m

% Get the number of seizures (columns)
numSeizures = size(spikesDuringSeizures, 2);

% Initialize a cell array to store correlation matrices for each seizure
correlationMatrices = cell(1, numSeizures);
...
for iSeizure= 1:numSeizures
    ...
    dataSeizure = spikesDuringSeizures(:, iSeizure);
   ...
   correlationMatrices{1,iSeizure} = correlation_matrix;     
end
%% Calculate average correlation from all seizures 
sumMatrix = zeros(size(correlationMatrices{1, 1})); % Initialize a matrix to accumulate the values
%countValidMatrices = 0;

cm = cell2mat(correlationMatrices);
cm = reshape(cm,numClusters,numClusters,numSeizures);
% Loop through each seizure matrix and accumulate the values
% for i = 1:numel(correlationMatrices)
%     currentMatrix = correlationMatrices{i};
%     
...
% averageMatrix = sumMatrix / countValidMatrices;

averageMatrix = nanmean(cm,3);
% Display the result
disp('Average Matrix:');
disp(averageMatrix);
