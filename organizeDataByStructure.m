function [organizedData,uniqueStructures] = organizeDataByStructure(lumpedAllResultsTable) 
% parent function: MasterCodePhaseEla20250212.m 

% IMPORTANT: here we swap right and left part so spike of the seizure is on
% the 50th bin 

uniqueStructures = unique(lumpedAllResultsTable.BrainStructure); 
organizedData = {}; % initiate the structure 
...
for iUniqBrain = 1:size(uniqueStructures,1)
    ...
    RowIdx = strcmp(currentStructure,lumpedAllResultsTable.BrainStructure); 
    filteredData = lumpedAllResultsTable(RowIdx == 1, :); %extract only rows that match this brain structure's name 
    ...
    % store the sums 
    organizedData{iUniqBrain,1} = currentStructure; 
    organizedData{iUniqBrain,2} = sumNON_SWDs; 
    organizedData{iUniqBrain,3} = sumSWDs; 
