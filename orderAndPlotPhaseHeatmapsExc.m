function [sortedUniqueStructures,neatDataWithPExc] = orderAndPlotPhaseHeatmapsExc(uniqueStructures,DataWithPExc,rowOrder,emptyRows)
...
DataWithPExc(emptyRows, :) = []; % Remove those rows from DataWithPInh 
...
% concatenate 
    ...
    for iUniqBrain = 1:size(uniqueStructures,1) 
        x = DataWithPExc{iUniqBrain,2}; % non SWDs 
        normalizedNON_SWDs = [normalizedNON_SWDs;x];
        y = DataWithPExc{iUniqBrain,3}; % SWDs 
...
figure; sgtitle('Non-SWDs norm ordered exc only'); % for non seizure 
...
figure; sgtitle('SWDs norm ordered exc only'); % for seizures 
for iUniqBrain = 1:numSubplots 
    currentStructure = sortedUniqueStructures{iUniqBrain}; % which structure now 
...
[neatDataWithPExc] = createNeatDataWithPExc(DataWithPExc,sortedNONSWDData,sortedData,rowOrder); % Reorganize DataWithPExc 
...
for iP = 1:size(neatDataWithPExc,1) 
    pValue = neatDataWithPExc{iP,4};
    if isnan(pValue) 
       neatDataWithPExc{iP,6} = NaN; 
    elseif pValue < 0.001
       neatDataWithPExc{iP,6} = '***'; % Highly significant
    elseif pValue < 0.01
        neatDataWithPExc{iP,6} = '**';  % Very significant
    elseif pValue < 0.05
        neatDataWithPExc{iP,6} = '*';   % Significant
    else
       neatDataWithPExc{iP,6} = 'n.s.'; % Not significant
    end
end 

% add to the plot 
% Add asterisks on the right side of each row
for i = 1:size(neatDataWithPExc,1)
    T = neatDataWithPExc{i,6}; % extract stars for this structure
    if isnan(T)
        T = 'NaN'; % if T is NaN change to a character variable NaN
    else
        T = neatDataWithPExc{i,6}; % if not, keep as it is
    end
    text(size(neatDataWithPExc, 2) + 1, i, T, 'FontSize', 10, 'FontWeight', 'normal', 'Color', 'k');
end
