% CODE FOR SPIKE-FIELD COHERENCE 

% Pick mouse of interest (Gria4 or Stargazer) 
mouse = 'Gria4'; % specify the mouse model in format mouse = 'Gria'; 
%mouse = 'Stargazer'; 
addpath '/media/elaX/intanData/ela/individualExperimentDataBase/2025_05_16__08_17_52'; % path to database 
...
homeFolder = '/media/elaX/MyFigures/SpikeFieldCoherence/June';  %  Make folder where figures will be saved 
...
for iMouse = 1:numMice
    ...
    % some seizures were excluded (too short). This line of code goes
    % through them and deletes empty rows 
    [COHR,SXX,SYY,SXY] = deleteEmptyRows(COHR,SXX,SYY,SXY); 
    numSeizures = size(COHR,2);  % new numSeizures (after getting rid of some) 
...
[reorderedAllFieldsLabelsDF] = orderedHeatmap(homeFolder, heatmapLabels,newHeatmapLabels,allFields,f); % plot the heatmap 
% plot dominant frequency only (added beg of Nov 2024) 
[allFields, heatmapLabels] = HeatmapDominantFrequencyOnly(DominantFrequency, f, labelsBigTable, whatMice); 
% plot dominant frequency but use the same order as for orderedHeatmap
% (added Nov '24) 
orderedDominantFrequencyOnly(reorderedAllFieldsLabelsDF, newHeatmapLabels, allFields); 
