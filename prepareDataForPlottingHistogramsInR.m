function prepareDataForPlottingHistogramsInR(strainOfInterest,neatDataWithP) 
...
%% Remove Dont Analyze 
idxDA = find(strcmp(neatDataWithP(:,1), 'DontAnalyze')); % Find the row index
neatDataWithP(idxDA, :) = []; % Remove the row

%% Save  S E I Z U R E  data for R 
% Convert cell array to a table
brainStructures = neatDataWithP(:,1); % First column (brain structure names)
SWDhistcounts = neatDataWithP(:,3); % Second column for seizure data (1x100 doubles)
...
% Save to CSV 
fileName = sprintf('histogramsSWDs_%s.csv', strainOfInterest); 
...
% save for R 
% Convert cell array to a table
nonSWDshistounts = neatDataWithP(:,2); % Second column for non seizures (1x100 doubles)
...
% Save to CSV
fileName = sprintf('histogramsNONSWDs_%s.csv', strainOfInterest); 
fullFilePath = fullfile(folderPath, fileName); % Combine folder and filename
writetable(dataTable, fullFilePath); % Save the file
