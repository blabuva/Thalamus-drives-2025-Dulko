function [Differences] = storeDifference(MeanDifference, combinations, Differences, iMouse)
...
for iPair= 1:size(MeanDifference,1)

structure1 = MeanDifference(iPair,1); 
%structure1 = structure1{1,1}; % unpack 

structure2 = MeanDifference(iPair,2); 
...
for iRow = 1:size(FirstColumnMatch,1) % loop through rows in search for '1' 
    ...
    if sumVal == 2 % means that the code found the match 
        ...
        differenceToBeStored = MeanDifference(iPair,3:size(MeanDifference,2)); 
        differenceToBeStored = cell2mat(differenceToBeStored); % convert to a numbers 
         
        % % Add a 'tail" of ones meanCorrelations so when the heatmap is
        % plotted we will be able to distinguish between mice) 
        differenceToBeStored = [differenceToBeStored, ones(1,1)];
        Differences{iRow,iMouse} = differenceToBeStored;
    end 
