function [spikesDuringSeizures] = CalculateFiringRateDuringSWS(StructureMouseDatabase, binSize)
...
if any(counts > 1)
    ...
    for iSeizure = 1:numSeizures 
               % analyze seizure by seizure starting from the first one 
                ...
                for iRow = 1:size(seizureData,1) % loop through lumped structures  
                       ...
                       numClusters = size(spikeTimes,1); 
                        ...
                        for iCluster = 1:numClusters
                            ...
                            spikesInRange = relativeOneSpikeTimes <= seizureDuration & relativeOneSpikeTimes >= 0; 
                            spikeTimesInRange = relativeOneSpikeTimes(spikesInRange); % Extract spike times within the time range
                
                            % Calculate the firing rate
                            edges = 0:binSize:seizureDuration;
                ...
                spikesDuringSeizures{iSeizure} = allNeuronsFR; % Store
                
    end
    numClusters = size(spikesDuringSeizures{1,1},1); % get final number of clusters 
        
    % % re-organize data so the output is in the same format as function after "else" below  
    [spikesDuringSeizures] = reOrganizeSpikesBeforeSeizures(spikesDuringSeizures,numSeizures,numClusters);
...
% Get spikes 
    ...
    spikesDuringSeizures = cell(numClusters, numSeizures);
    ...
    for iSeizure = 1:numSeizures
        ...
        for iCluster = 1:numClusters
            ...
            % Store firing rate in the corresponding cell
            spikesDuringSeizures{iCluster, iSeizure} = firingRate;
        end
    end
end
