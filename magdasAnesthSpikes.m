...
for iPart = 1:length(brainParts)
    ...
    for iRow = 1:size(mappedKSUnitsMod.singleUnits, 1)
        if  strcmp(currentPart, mappedKSUnitsMod.singleUnits.Channel_Brain(iRow))
            ...
            clusterAssignment = mappedKSUnitsMod.singleUnits.Assignment{iRow} ;
            ...
            if iNeuron == 1
                ...
                spikesStructure.(brainPartFieldName).binnedSpikes = array2table(clusterID, 'variablenames', {'ClusterID'}) ;
            else
                spikesStructure.(brainPartFieldName).spikeTimes.ClusterID(iNeuron,1) = clusterID ;
                spikesStructure.(brainPartFieldName).binnedSpikes.ClusterID(iNeuron,1) = clusterID ;
            end
            spikesStructure.(brainPartFieldName).spikeTimes.Assigment{iNeuron,1} = clusterAssignment ;
            spikesStructure.(brainPartFieldName).binnedSpikes.Assigment{iNeuron,1} = clusterAssignment ;
            ...
            iNeuron = iNeuron + 1 ;
            clear spikeTimes binnedSpikes clusterID clusterAssignment
        end
    end
