...
%% temp checker to see if multis are getting counted
% phaseFields = fieldnames(phase.gria) ;
% phaseMultiChecker = table ;
% for iStructure = 1:length(phaseFields)
%     phaseMultiChecker.Structure{iStructure} = phaseFields{iStructure} ;
%     phaseMultiChecker.singleSize(iStructure) = size(phase.gria.(phaseFields{iStructure}).singleALL.perSWD.matNoBlanks, 1) ;
% 
% end
