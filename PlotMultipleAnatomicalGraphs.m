function PlotMultipleAnatomicalGraphs(LabelsBigHeatmap_DiffSorted,rowMeans)
...
%% 2. Loop through tresholds and plot for each of them 
figure; 
...
for iTr = 1:size(thresholds,2)
    ...
    subplot(2,2,iTr); % plot the graph for each threshold 
        plotAnatomicalGraph(correlations,brainStructures,edgeNamesFiltered,y,x)
        title(sprintf('Threshold = %.2f', currentThreshold));
