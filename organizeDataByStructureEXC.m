function [organizedDataExc,uniqueStructures] = organizeDataByStructureEXC(lumpedAllResultsExcTable) 
% parent function: masterExcPlotter.m 

% IMPORTANT: here we swap right and left part so spike of the seizure is on
% the 50th bin 

uniqueStructures = unique(lumpedAllResultsExcTable.BrainStructure); 
organizedDataExc = {}; % initiate the structure 
...
for iUniqBrain = 1:size(uniqueStructures,1)
    ...
    RowIdx = strcmp(currentStructure,lumpedAllResultsExcTable.BrainStructure); 
    filteredData = lumpedAllResultsExcTable(RowIdx == 1, :); %extract only rows that match this brain structure's name 
    ...
    % store the sums 
    organizedDataExc{iUniqBrain,1} = currentStructure; 
    organizedDataExc{iUniqBrain,2} = sumNON_SWDs; 
    organizedDataExc{iUniqBrain,3} = sumSWDs; 
